...
const TechstackComponent = () => {
    ...
    const hoverHandler = (prop,techIn) => {
        ...
        setTechnologies(prevTechnologies =>
            prevTechnologies.map((tech, index) =>
              tech === techIn ? { ...tech, saturation: !status } : tech
            )
        )   
    }
    ...
    technologies.map((tech) => {
        return <div><img src={tech.image} alt={tech.name} key={tech.name} style= {{padding: '24px', height: '128px',transform: `scale(${tech.saturation ? 0.95 : 1})` , transitionDuration: '500ms', filter: `saturate(${tech.saturation ? 150 : 0}%)`, transition: 'filter 0.5s linear', background: tech.saturation ? `radial-gradient(${tech.saturation ? `${tech.gradientHigh} 15%` : 'transparent'},  transparent, transparent)`: 'transparent'}} onMouseLeave={() => hoverHandler(this, tech)} onMouseOver={() => hoverHandler(this, tech)} /></div>  
    })
    }
    </div>
    </>
